...
export default function AboutSection({ themeMode }) {
  ...
  const glowColor = themeMode === 'upside-down' ? '48, 86, 211' : '229, 9, 20'
    ...
    <section id="about" className="py-24">
      ...
      <div className="relative w-full">
        <MagicBento
          ...
          enableTilt={false}
          className="grid gap-8 md:grid-cols-12 items-stretch max-w-none p-0"
        >
          ...
          >
            <ParticleCard
              glowColor={glowColor}
              className="h-full w-full neon-frame"
              particleCount={8}
              enableTilt={false} // TiltedCard handles tilt
          ...
          >
            <ParticleCard
              glowColor={glowColor}
              className={`card card--border-glow glass-panel p-8 md:p-10 relative overflow-hidden group min-h-[400px] flex flex-col justify-center ${themeMode === 'upside-down' ? 'border-stBlue/40' : 'border-stRed/40'}`}
              enableTilt={false}
              enableMagnetism={false}
              particleCount={15}
            >
